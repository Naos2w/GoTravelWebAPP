...
const TimePicker: React.FC<{
  ...
  return (
    <div ref={ref} className="absolute top-full left-1/2 -translate-x-1/2 sm:left-0 sm:translate-x-0 mt-2 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-[28px] shadow-2xl z-[100] p-4 flex gap-4 animate-in fade-in zoom-in-95 duration-200">
...
export const Itinerary: React.FC<Props> = ({ trip, onUpdate }) => {
  ...
  const calculateTransportTime = async (index: number, transportId: string, type: TransportType, currentItems: ItineraryItem[]) => {
    ...
    try {
      ...
      const prompt = `Estimate transport time from "${prev.placeName}" to "${next.placeName}" by "${type}". Return ONLY duration like "20 mins". Language: ${language === 'zh' ? 'Traditional Chinese' : 'English'}.`;
      const response = await ai.models.generateContent({ 
        model: 'gemini-3-flash-preview', 
        contents: prompt,
        config: { tools: [{ googleSearch: {} }] }
      });
      const resultText = response.text?.trim().replace(/^約\s*/, '') || labels.noData;
      
      const updatedItems = currentItems.map(it => it.id === transportId ? { ...it, note: resultText, transportType: type } : it);
      saveItineraryUpdate(updatedItems);
    } catch {
      const duration = DateTimeUtils.getDuration(prev.time, next.time);
      const updatedItems = currentItems.map(it => it.id === transportId ? { ...it, note: duration || labels.noData, transportType: type } : it);
      saveItineraryUpdate(updatedItems);
    } finally {
  ...
  const updateItem = async (itemIdx: number, field: keyof ItineraryItem, value: any) => {
    ...
    if (field === 'time') {
      ...
      if (newItems[newIdx - 1]?.type === 'Transport' && newItems[newIdx - 1]?.transportType !== 'Flight') {
         calculateTransportTime(newIdx - 1, newItems[newIdx - 1].id, newItems[newIdx - 1].transportType!, newItems);
      }
      if (newItems[newIdx + 1]?.type === 'Transport' && newItems[newIdx + 1]?.transportType !== 'Flight') {
         calculateTransportTime(newIdx + 1, newItems[newIdx + 1].id, newItems[newIdx + 1].transportType!, newItems);
      }
    ...
    <div className="flex flex-col lg:flex-row gap-4 lg:gap-8 h-[calc(100vh-160px)] animate-in fade-in duration-500 overflow-hidden">
        ...
        {days.map((day, idx) => {
          ...
          return (
            <button key={idx} onClick={() => { setSelectedDayIndex(idx); setInsertingAt(null); }} className={`flex-shrink-0 lg:w-full p-2.5 lg:p-3.5 rounded-2xl text-center transition-all duration-300 border ${isSelected ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-lg border-slate-900 dark:border-white' : 'bg-white dark:bg-slate-800 text-slate-500 border-gray-100 dark:border-slate-800 hover:bg-slate-50 shadow-sm'}`}>
              ...
              <div className="font-bold text-sm leading-tight">{d.toLocaleDateString(language === 'zh' ? 'zh-TW' : 'en-US', { month: 'numeric', day: 'numeric' })}</div>
            </button>
          );
        })}
      </div>
      ...
      <div className="flex-1 bg-white dark:bg-slate-900 rounded-[28px] lg:rounded-[40px] shadow-sm border border-gray-100 dark:border-slate-800 flex flex-col overflow-hidden">
        <div className="p-4 sm:p-5 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center bg-white/80 dark:bg-slate-900/80 backdrop-blur-md sticky top-0 z-20">
          <h2 className="text-lg font-black text-slate-900 dark:text-white">Day {selectedDayIndex + 1}</h2>
          <div className="flex bg-slate-100/50 dark:bg-slate-800 p-1 rounded-xl border border-gray-100 dark:border-slate-700">
            <button onClick={() => addActivity('Place')} className="px-3 py-1.5 hover:bg-white dark:hover:bg-slate-700 rounded-lg text-primary font-black text-[11px] flex items-center gap-1.5 transition-all"><MapPin size={12}/> <span className="hidden sm:inline">{labels.addPlace}</span></button>
            <button onClick={() => addActivity('Food')} className="px-3 py-1.5 hover:bg-white dark:hover:bg-slate-700 rounded-lg text-orange-500 font-black text-[11px] flex items-center gap-1.5 transition-all ml-1"><Coffee size={12}/> <span className="hidden sm:inline">{labels.addFood}</span></button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-0 relative custom-scrollbar">
          {currentDay?.items.map((item, idx) => {
            ...
            const canInsertTransport = nextItem && !isTransport && nextItem.type !== 'Transport';

            return (
              <React.Fragment key={item.id}>
                <div className={`relative pl-10 sm:pl-12 ${isTransport ? 'pb-2 sm:pb-3' : 'pb-6 sm:pb-8'}`}>
                  {!isTransport && (
                  ...
                  {isTransport ? (
                    <div className={`group bg-slate-50 dark:bg-slate-800/40 rounded-xl px-3 py-1.5 border border-dashed flex flex-col sm:flex-row items-center justify-center sm:justify-between h-auto sm:h-11 py-2 sm:py-0 relative ${isFlight ? 'bg-blue-50/20 border-blue-100 dark:border-blue-900/20' : 'border-slate-200 dark:border-slate-700 hover:border-slate-400'} transition-all`}>
                       <div className="flex items-center gap-1.5 relative pr-10 sm:pr-0">
                          <button 
                            disabled={isFlight || calculatingId === item.id}
                            onClick={() => !isFlight && setShowTransportPickerId(item.id)}
                            className={`flex items-center gap-1.5 p-1 rounded-lg transition-colors ${isFlight ? 'cursor-default' : 'hover:bg-slate-100 dark:hover:bg-slate-700'}`}
                          >
                            ...
                            <span className={`text-[10px] sm:text-xs font-bold truncate ${isFlight ? 'text-blue-600 dark:text-blue-400' : 'text-slate-500 dark:text-slate-400'}`}>
                               <span className="hidden sm:inline">{labels.moving}: </span>
                               {calculatingId === item.id ? <Loader2 size={10} className="animate-spin" /> : item.note}
                            </span>
                          </button>
                          
                          {showTransportPickerId === item.id && (
                            <div ref={editRef} className="absolute bottom-full left-0 mb-2 z-[110] bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl shadow-2xl p-2 flex gap-1 animate-in zoom-in-95 duration-200">
                               {TRANSPORT_OPTIONS.map(opt => (
                                 <button 
                                    key={opt.type} 
                                    onClick={() => { setShowTransportPickerId(null); calculateTransportTime(idx, item.id, opt.type, currentDay.items); }} 
                       ...
                       {!isFlight && (
                         <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col sm:flex-row gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                            <button onClick={() => deleteItem(idx)} className="p-1.5 bg-white dark:bg-slate-700 text-red-500/80 hover:text-red-600 rounded-lg shadow-sm border border-slate-100 dark:border-slate-600"><Trash2 size={12}/></button>
                         </div>
                       )}
                    </div>
                  ) : (
                    <div className={`group bg-white dark:bg-slate-800/80 p-3 sm:p-3.5 rounded-xl sm:rounded-2xl border border-transparent shadow-ios hover:shadow-ios-lg transition-all relative ${isFlight ? 'ring-1 ring-blue-500/20 bg-blue-50/5' : ''}`}>
                      <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 items-center sm:items-center pr-12 sm:pr-32">
                        <div className="shrink-0 relative">
                          <button 
                            disabled={isFlight}
                            onClick={() => !isFlight && setShowTimePickerId(item.id)}
                            className={`font-mono font-black text-xs sm:text-sm focus:outline-none transition-colors ${isFlight ? 'text-blue-600 dark:text-blue-400 cursor-default' : 'text-slate-900 dark:text-white hover:text-primary'}`}
                          >
                            {item.time}
                          </button>
                          {showTimePickerId === item.id && (
                            <TimePicker 
                              value={item.time} 
                              onChange={(t) => { updateItem(idx, 'time', t); setShowTimePickerId(null); }} 
                        ...
                        <div className="flex-1 space-y-0.5 min-w-0 flex flex-col items-center sm:items-start text-center sm:text-left">
                          <div className="flex items-center gap-1.5 w-full justify-center sm:justify-start">
                             {isFlight && <Plane size={12} className="text-blue-500 shrink-0" />}
                             <SafeInput 
                                disabled={isFlight} 
                                value={item.placeName} 
                                onChange={(val) => updateItem(idx, 'placeName', val)} 
                                className={`font-black text-sm sm:text-base bg-transparent border-none w-full p-0 focus:ring-0 truncate text-center sm:text-left ${isFlight ? 'text-blue-900 dark:text-blue-300' : item.type === 'Food' ? 'text-orange-600' : 'text-slate-900 dark:text-white'}`} 
                             />
                          </div>
                          <SafeInput 
                            disabled={isFlight} 
                      ...
                      <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col sm:flex-row gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                         <button onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.placeName)}`, '_blank')} className="p-1.5 bg-slate-50 dark:bg-slate-700 text-slate-400 hover:text-primary hover:bg-primary/5 rounded-lg shadow-sm border border-slate-100 dark:border-slate-600"><Map size={14} /></button>
                         {!isFlight && (
                           <button onClick={() => deleteItem(idx)} className="p-1.5 bg-slate-50 dark:bg-slate-700 text-red-400 hover:text-red-500 hover:bg-red-500/5 rounded-lg shadow-sm border border-slate-100 dark:border-slate-600"><Trash2 size={14} /></button>
                         )}
                      </div>
                    </div>
                  )}
                ...
                {canInsertTransport && (
                  <div className="relative flex items-center justify-center py-2 group/btn">
                    ...
                    {insertingAt === idx ? (
                      <div ref={editRef} className="z-[110] bg-white dark:bg-slate-800 border border-indigo-100 dark:border-indigo-900 shadow-xl rounded-2xl p-1.5 flex items-center gap-1 animate-in zoom-in-95 duration-200">
                        {TRANSPORT_OPTIONS.map(opt => (
