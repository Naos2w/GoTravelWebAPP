...
const ShareModal: React.FC<{
  ...
  // Is the current user the one who created this trip?
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl p-2 space-y-1 max-h-40 overflow-y-auto custom-scrollbar">
              ...
              <div className="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-black text-slate-900 dark:text-white truncate">
                    {isOwner ? t("youOwner") : t("owner")}
                  </div>
                </div>
              </div>

              {/* Allowed Emails (Guests Only) */}
              {trip.allowed_emails &&
                trip.allowed_emails
                  .filter((e) => e !== user.email) // Ensure owner is hidden from guest list even if in array
                  .map((e, i) => (
                    <div
                      key={i}
              ...
              {(!trip.allowed_emails ||
                trip.allowed_emails.filter((e) => e !== user.email).length ===
                  0) && (
                <div className="p-4 text-center text-[10px] font-bold text-slate-300 uppercase tracking-widest">
                  {t("noUpcoming")}
                </div>
              )}
            </div>
...
const App: React.FC = () => {
  ...
  const handleAuthUser = (supabaseUser: any) => {
    ...
    if (new URLSearchParams(window.location.search).get("tripId"))
      setView("detail");
    else setView("list");
  };
  ...
  // Flight Info Extraction (Hero Section)
  const allFlights = currentTrip?.flights || [];
  const arrivalCode = allFlights[0]?.outbound?.arrivalAirport || "";
  const uniqueOutFlightNos = useMemo(
    () =>
      Array.from(
        new Set(allFlights.map((f) => f.outbound.flightNumber).filter(Boolean))
      ),
    [allFlights]
  );
  const uniqueInFlightNos = useMemo(
    () =>
      Array.from(
        new Set(allFlights.map((f) => f.inbound?.flightNumber).filter(Boolean))
      ),
    [allFlights]
  );
  ...
  const chartData = useMemo(() => {
    ...
    if (flightsSum > 0)
      dataMap["Flight"] = (dataMap["Flight"] || 0) + flightsSum; // Use 'Flight' category
  ...
  const MobileHero = () => (
    <div className="md:hidden space-y-4 mb-6">
      {/* Updated Hero Card - Dynamic Content */}
      <div
        className={`w-full rounded-[32px] p-8 shadow-ios border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800`}
      >
        <div className="flex flex-col gap-2">
          <div className="flex justify-between items-start mb-1">
            ...
            <div className="flex flex-col items-end gap-1">
              {uniqueOutFlightNos.length > 0 && (
                <div className="text-[9px] font-black bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded-lg text-slate-600 dark:text-white flex items-center gap-1">
                  <Plane size={9} /> {uniqueOutFlightNos.join(", ")}
                </div>
              )}
              {uniqueInFlightNos.length > 0 && (
                <div className="text-[9px] font-black bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded-lg text-slate-400 dark:text-slate-400 flex items-center gap-1">
                  <Plane size={9} className="rotate-180" />{" "}
                  {uniqueInFlightNos.join(", ")}
                </div>
              )}
            </div>
          </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-[32px] p-6 shadow-ios border border-slate-100 dark:border-slate-700">
        ...
        <div className="space-y-4">
          {upcomingItems.slice(0, 5).map((item, i) => (
            <div
              key={i}
              className="flex gap-4 group cursor-pointer active:scale-[0.98] active:bg-slate-50 dark:active:bg-slate-700/50 rounded-xl transition-all p-1 -m-1"
              onClick={() =>
                window.open(
          ...
          {upcomingItems.length === 0 && (
            <div className="text-slate-300 font-bold text-xs uppercase text-center py-4">
              {t("noUpcoming")}
            </div>
          )}
        </div>
      ...
      {!isGuest && (
        <button
          ...
          className="w-full py-4 flex items-center justify-center gap-2 text-red-500 font-black text-xs uppercase tracking-widest bg-white dark:bg-slate-800 border border-red-100 dark:border-red-900/30 rounded-2xl shadow-sm active:scale-95 transition-all"
        >
      ...
      <div className="min-h-screen bg-background-light dark:bg-background-dark flex flex-col items-center justify-center p-8 text-center space-y-6">
        ...
        <div>
          ...
          {user && (
            <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-2xl max-w-sm mx-auto border border-slate-100 dark:border-slate-800">
              ...
              <div className="font-mono text-sm font-bold text-slate-800 dark:text-white select-all">
                {user.email}
              </div>
            </div>
          )}
        ...
        <div className="flex gap-3">
          <button
            ...
            className="px-8 py-3 rounded-2xl font-black text-sm text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all"
          >
            {t("backHome")}
          </button>
          <button
            onClick={handleManualJoin}
            className="bg-primary text-white px-8 py-3 rounded-2xl font-black text-sm transition-all active:scale-95 shadow-lg shadow-primary/20"
          >
            {t("join")}
          </button>
        </div>
              ...
              {activeTab === "dashboard" && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-700">
                  ...
                  <div className="hidden md:grid grid-cols-12 grid-rows-2 gap-6 h-[700px]">
                    {/* 1. Main Hero - Now showing Airport Code and Combined Flight Nos */}
                    <div
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="mt-6 flex flex-wrap gap-2">
                          {uniqueOutFlightNos.map((fn) => (
                            <div
                              key={fn}
                              className="text-[10px] font-black bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-4 py-2 rounded-xl flex items-center gap-2 shadow-lg shadow-slate-200 dark:shadow-none transition-transform hover:scale-105"
                            >
                              <Plane size={12} /> {fn}
                            </div>
                          ))}
                          {uniqueInFlightNos.map((fn) => (
                            <div
                              key={fn}
                              className="text-[10px] font-black bg-slate-100 dark:bg-slate-700 px-4 py-2 rounded-xl text-slate-400 flex items-center gap-2 transition-transform hover:scale-105"
                            >
                    ...
                    <div className="col-span-3 row-span-1 bg-white dark:bg-slate-800 rounded-[40px] p-6 border border-slate-100 dark:border-slate-700 shadow-ios flex flex-col relative group/budget">
                      <div className="flex justify-between items-start mb-4">
                        ...
                        {!isGuest && (
                          <button
                            ...
                            className="p-2 text-slate-300 hover:text-indigo-500 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-all"
                          >
                            <Edit2 size={16} />
                          </button>
                        )}
                      ...
                      <div className="flex-1 flex flex-col justify-end">
                        ...
                        <div className="flex justify-between mt-2 text-xs font-bold text-slate-400">
                          <span>
                            {t("spent")}: ${(spentTotal / 1000).toFixed(1)}k
                          </span>
                          <span>
                            {t("limit")}: ${(budgetLimit / 1000).toFixed(1)}k
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="col-span-4 row-span-1 flex flex-col gap-6">
                      ...
                      {!isGuest && (
                        <button
                          onClick={handleDeleteTrip}
                          className="h-16 flex items-center justify-center gap-2 text-red-500 font-black text-xs uppercase tracking-widest bg-white dark:bg-slate-800 border border-red-50 dark:border-red-900/30 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-3xl transition-all shadow-sm"
                        >
                          <Trash2 size={16} /> {t("deleteTrip")}
                        </button>
                      )}
                ...
                <div className="bg-white dark:bg-slate-800 w-full max-w-sm rounded-[32px] p-6 shadow-2xl animate-in slide-in-from-bottom-10">
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className="flex-1 py-3 text-slate-500 font-bold bg-slate-100 dark:bg-slate-700 rounded-xl"
                    >
                      {t("cancel")}
                    </button>
                    <button
                      onClick={saveBudget}
                      className="flex-1 py-3 text-white font-bold bg-primary rounded-xl"
                    >
                      {t("save")}
                    </button>
                  </div>
          ...
          <div className="p-6 sm:p-12 max-w-7xl mx-auto min-h-screen">
            ...
            {error && (
              <div className="p-6 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-3xl flex items-center gap-4 mb-8 animate-in fade-in slide-in-from-top-4">
                ...
                <div>
                  <h3 className="font-black text-red-600 dark:text-red-400">
                    {t("errorTitle")}
                  </h3>
                  <p className="text-xs font-bold text-red-400 dark:text-red-500/80">
                    {t("errorDesc")} ({error})
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
              {trips
                .filter(
                  (t) =>
                    t.user_id === user?.id ||
                    t.allowed_emails?.includes(user?.email || "")
                )
                .map((trip) => (
                  <div
                    key={trip.id}
