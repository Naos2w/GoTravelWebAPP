...
import { 
  ...
  Ticket, ShoppingBag, Tag, Clock, ChevronDown, X as CloseIcon
} from 'lucide-react';
...
const CATEGORY_CONFIG: Record<string, { icon: any, color: string, bgColor: string, darkBgColor: string, textColor: string }> = {
  ...
  Other: { icon: Tag, color: 'bg-slate-400', bgColor: 'bg-slate-100', darkBgColor: 'dark:bg-slate-800/50', textColor: 'text-slate-500' }
};
           ...
           <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isFormOpen ? 'max-h-[600px] opacity-100 py-8 px-6 sm:px-10' : 'max-h-0 opacity-0'}`}>
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
               <div className="relative">
                  ...
                  <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0" className="w-full pl-10 pr-4 py-3 mt-1 bg-white dark:bg-slate-800 dark:text-white rounded-2xl border border-slate-100 dark:border-slate-700 focus:border-primary/20 outline-none font-black text-sm shadow-sm transition-all" />
               </div>
               <select value={currency} onChange={e => setCurrency(e.target.value as Currency)} className="py-3 px-4 mt-1 bg-white dark:bg-slate-800 dark:text-white rounded-2xl border border-slate-100 dark:border-slate-700 outline-none font-black text-sm shadow-sm transition-all cursor-pointer">
                 {Object.values(Currency).map(c => <option key={c} value={c}>{c}</option>)}
               </select>
               <select value={selectedDate} onChange={e => setSelectedDate(e.target.value)} className="py-3 px-4 mt-1 bg-white dark:bg-slate-800 dark:text-white rounded-2xl border border-slate-100 dark:border-slate-700 outline-none font-black text-sm shadow-sm transition-all cursor-pointer">
                 {dateOptions.map(d => <option key={d} value={d}>{d}</option>)}
               </select>
               <select value={category} onChange={e => setCategory(e.target.value as any)} className="py-3 px-4 mt-1 bg-white dark:bg-slate-800 dark:text-white rounded-2xl border border-slate-100 dark:border-slate-700 outline-none font-black text-sm shadow-sm transition-all cursor-pointer">
                 {Object.keys(CATEGORY_CONFIG).map(c => <option key={c} value={c}>{(labels as any)[c]}</option>)}
               </select>
             </div>
             <input type="text" value={note} onChange={e => setNote(e.target.value)} placeholder={labels.desc} required className="w-full mt-4 px-6 py-3 bg-white dark:bg-slate-800 dark:text-white rounded-2xl text-sm font-bold border border-slate-100 dark:border-slate-700 outline-none shadow-sm transition-all" />
             <div className="flex gap-3 mt-6">
               <button onClick={addExpense} className="flex-1 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl font-black py-3.5 hover:scale-[1.01] active:scale-95 transition-all shadow-xl shadow-slate-200 dark:shadow-none">{labels.addBtn}</button>
               <button onClick={() => setIsFormOpen(false)} className="px-8 py-3.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-2xl font-black transition-all">{labels.cancel}</button>
             </div>
           </div>
        </div>
        ...
        <div className="px-6 sm:px-10 py-4 flex flex-col sm:flex-row gap-4 items-center justify-between border-b border-gray-100 dark:border-slate-700 bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm sticky top-0 z-10">
           <div className="flex items-center gap-2 overflow-x-auto custom-scrollbar w-full sm:w-auto pb-2 sm:pb-0 whitespace-nowrap snap-x">
              ...
              {Object.keys(CATEGORY_CONFIG).map(cat => (
                <button key={cat} onClick={() => setFilterCategory(cat)} className={`flex-shrink-0 px-3.5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all snap-start flex items-center gap-2 ${filterCategory === cat ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-lg' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 bg-slate-100 dark:bg-slate-700'}`}>
           ...
           <div className="relative group w-full sm:w-56 shrink-0">
              <div className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-700 cursor-pointer relative transition-colors hover:bg-slate-100 dark:hover:bg-slate-800">
                 ...
                 <select value={sortOrder} onChange={e => setSortOrder(e.target.value as SortType)} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10 bg-white dark:bg-slate-900">
                 ...
                 <div className="flex-1 text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-300 pointer-events-none truncate">
                    {sortOrder === 'created-desc' ? labels.createdDesc : sortOrder === 'created-asc' ? labels.createdAsc : sortOrder === 'amount-desc' ? labels.amountHigh : labels.amountLow}
                 </div>
                 <ChevronDown size={14} className="text-slate-300 dark:text-slate-600 pointer-events-none" />
              </div>
           </div>
        </div>
